...
<!-- Bootstrap Navbar -->
	...
	<?php
		...
		$_SESSION['port'] = $_POST['ports'];
		// Select all fields from device_ports table where device = selected device and the portID = select port
		$stmt = $dsn->query("SELECT * FROM device_ports WHERE deviceName = '{$_SESSION['device']}' AND portID = '{$_SESSION['port']}'");
	?>
